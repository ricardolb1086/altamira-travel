...
function renderItinerary(data) {
  ...
  const flights = (data.flights || []).filter(flight => flight.from || flight.to || flight.airline).map(flight => `<div style="border:1px solid #ded7ca;border-radius:5px;padding:15px;margin:0 0 10px"><strong style="font-family:Georgia,serif;font-size:18px;font-weight:normal">${escapeHTML(flight.from || 'Origen')} → ${escapeHTML(flight.to || 'Destino')}</strong><div style="font-size:12px;color:#6b6258;margin-top:5px">${escapeHTML([flight.airline, flight.number, flight.date, flight.depart && flight.arrive ? `${flight.depart} – ${flight.arrive}` : ''].filter(Boolean).join(' · '))}</div>${flight.notes ? `<p style="font-size:12px;line-height:1.5;color:#a85e32;margin:8px 0 0">${escapeHTML(flight.notes)}</p>` : ''}</div>`).join('');
  ...
  const pricing = price || airfare ? `<div style="background:#2e2820;color:#fff8ed;padding:24px;margin:28px 0"><table role="presentation" style="width:100%;border-collapse:collapse"><tr>${price ? `<td style="padding:0 12px 0 0"><span style="font-size:9px;letter-spacing:1.5px;color:#d8cbbc">PROGRAMA POR PERSONA</span><div style="font-family:Georgia,serif;font-size:25px;color:#df9a70;margin-top:6px">${currency} ${price.toLocaleString('en-US')}</div></td>` : ''}${airfare ? `<td style="padding:0 12px"><span style="font-size:9px;letter-spacing:1.5px;color:#d8cbbc">VUELOS POR PERSONA</span><div style="font-family:Georgia,serif;font-size:25px;color:#df9a70;margin-top:6px">${currency} ${airfare.toLocaleString('en-US')}</div></td><td style="padding:0 0 0 12px"><span style="font-size:9px;letter-spacing:1.5px;color:#d8cbbc">TOTAL ESTIMADO</span><div style="font-family:Georgia,serif;font-size:25px;color:#fff8ed;margin-top:6px">${currency} ${total.toLocaleString('en-US')}</div></td>` : ''}</tr></table>${data.pricing?.fareNotice ? `<p style="border-top:1px solid #4c443b;margin:18px 0 0;padding-top:14px;font-size:11px;line-height:1.5;color:#d8cbbc">${escapeHTML(data.pricing.fareNotice)}</p>` : ''}</div>` : '';
  const html = `${data.trip?.summary ? `<p style="font-family:Georgia,serif;font-size:19px;line-height:1.65;color:#5f574e">${escapeHTML(data.trip.summary)}</p>` : ''}<h2 style="font-family:Georgia,serif;font-size:30px;font-weight:normal;margin:34px 0 5px">El viaje, día a día</h2>${days}${flights ? `<h2 style="font-family:Georgia,serif;font-size:30px;font-weight:normal;margin:34px 0 14px">Vuelos</h2>${flights}` : ''}${pricing}${includes ? `<h3 style="font-family:Georgia,serif;font-size:23px;font-weight:normal">El viaje incluye</h3><ul style="padding-left:20px;font-size:13px;line-height:1.5">${includes}</ul>` : ''}${excludes ? `<h3 style="font-family:Georgia,serif;font-size:23px;font-weight:normal">No incluye</h3><ul style="padding-left:20px;font-size:13px;line-height:1.5">${excludes}</ul>` : ''}`;
