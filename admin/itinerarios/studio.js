(() => {
  ...
  const defaults = () => ({
    ...
    pricing: { currency: 'USD', price: '', deposit: '', validUntil: '', terms: '' },
  ...
  function renderPreview() {
    const t=state.trip, price=Number(state.pricing.price||0), currency=state.pricing.currency;
    ...
    $('#proposal').innerHTML=`<header class="proposal-cover ${t.cover?'has-image':''}"${coverStyle}><div class="proposal-logo">ALTA<b>MIRA</b> · TRAVEL</div><div><div class="proposal-kicker">Itinerario personalizado</div><h1>${escapeHTML(t.title||'Tu próximo gran viaje')}</h1><div class="proposal-route">${escapeHTML(t.route||'Una experiencia diseñada a tu medida')}</div><div class="proposal-client">Preparado para ${escapeHTML(t.client||'nuestro viajero')}</div></div></header>
      ...
      ${price?`<section class="proposal-price"><div><small>Inversión por persona</small><br><span>${state.pricing.deposit?`Reserva con ${escapeHTML(state.pricing.deposit)}`:''}</span></div><strong>${currency} ${price.toLocaleString('en-US')}</strong></section>`:''}
      ...
      <footer class="proposal-footer"><div><strong>ALTA<span style="color:#c47646">MIRA</span> TRAVEL</strong><div class="eyebrow">Journeys made unforgettable</div></div><p>${escapeHTML(state.contact.closing)}<br><b>${escapeHTML(state.contact.name)}</b><br>${escapeHTML(state.contact.email)} · ${escapeHTML(state.contact.phone)}</p></footer>`;
  }
  function setupNav() {
